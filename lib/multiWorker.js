...
class MultiWorker extends EventEmitter {
  constructor (options, jobs) {
    ...
    setInterval(() => {
      this.eventLoopBlocked = false
      this.eventLoopCheckCounter++
      this.eventLoopDelay = h.max / 1000 / 1000 // nano -> micro -> milli
  ...
  async startWorker () {
    ...
    worker.on('pause', (timeout) => { this.emit('pause', worker.id) })
    worker.on('paused', (timeout) => { this.emit('paused', worker.id) })
    worker.on('resume', () => { this.emit('resume', worker.id) })
    worker.on('sleep', () => { this.emit('sleep', worker.id) })

    worker.on('redis_connect', (options) => { this.emit('redis_connect', options, worker.id) })
    worker.on('redis_close', (options) => { this.emit('redis_close', options, worker.id) })
    worker.on('redis_reconnecting', (options) => { this.emit('redis_reconnecting', options, worker.id) })
  ...
  async checkWrapper () {
    ...
    this.checkTimer = setTimeout(this.checkWrapper.bind(this), this.options.checkTimeout)
  }

  async start () {
    this.running = true

    // Immediately start up min_task_processors worth of workers
    if (this.options.minTaskProcessors > 1) {
      for(var i=0; i < this.options.minTaskProcessors; i++) {
        await this.startWorker()
      }
    }
  ...
  untrack () {
    return Promise.all(this.workers.map(worker => {
      return worker.untrack()
    }))
  }

  pause () {
    return Promise.all(this.workers.map(worker => {
      return worker.setPause()
    }))
  }
