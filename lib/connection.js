...
class Connection extends EventEmitter {
  ...
  async connect () {
    ...
    if (this.options.redis) {
      ...
      if (this.options.pkg === 'ioredis') {
        ...
        if (typeof this.options.dsn === 'string') {
          console.log('> Connecting with DSN:', this.options.dsn)
          this.redis = new Pkg(this.options.dsn, this.options.options)
        } else {
          console.log('> Connecting with host/port:', this.options.host, this.options.port)
    ...
    // TODO: This is valuable everywhere but production
